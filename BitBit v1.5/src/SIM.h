...
namespace SIMD {
  ...
  inline float reduceAddPs(VecF vec) { return _mm512_reduce_add_ps(vec); }
  ...
  inline int HaddEpi32(VecI v) {
    VecI sum64 = _mm_add_epi32(v, _mm_unpackhi_epi64(v, v));
    return _mm_cvtsi128_si32(_mm_add_epi32(sum64, _mm_shuffle_epi32(sum64, _MM_SHUFFLE(2, 3, 0, 1))));
  }

#endif

  constexpr int Alignment = std::max(8, static_cast<int>(sizeof(VecI)));

  inline VecI dpbusdEpi32(VecI sum, VecI x, VecI y) {
    return addEpi32(sum, maddEpi16(maddubsEpi16(x, y), set1Epi16(1)));
  }
