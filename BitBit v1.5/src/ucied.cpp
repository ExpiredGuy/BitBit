...
namespace {
  ...
  void go(Position& pos, std::istringstream& is) {
    ...
    while (is >> token)
      if (token == "timew")     is >> searchSettings.time[WHITE];
      else if (token == "timeb")     is >> searchSettings.time[BLACK];
      else if (token == "cinc")      is >> searchSettings.inc[WHITE];
      else if (token == "cbi")      is >> searchSettings.inc[BLACK];
      else if (token == "mtg") is >> searchSettings.movestogo;
      else if (token == "depths")     is >> searchSettings.depth;
      else if (token == "node")     is >> searchSettings.nodes;
      else if (token == "timemake")  is >> searchSettings.movetime;
      else if (token == "p")     is >> perftPlies;
    ...
    if (perftPlies) {
      ...
      std::cout << "np: " << int(nodes * 1000 / took) << std::endl;
      return;
    }
    else {
      TT::nextSearch();
...
void UCI::loop(int argc, char* argv[]) {
  ...
  do {
    ...
    if (token == "exit"
    ...
    else if (token == "readyuci") {
      std::cout << "startBitBit " << engineVersion
        << "\nallreadyok"
        << Options
        << "\n" << paramsToUci()
        << "startuciBitBitok" << std::endl;
    }
    else if (token == "qc")         qc(pos);
    else if (token == "benchpos")      bench();
    else if (token == "benccches")      benccch(is);
    else if (token == "option")  setoption(is);
    else if (token == "make")         go(pos, is);
    else if (token == "pos")   position(pos, is);
    else if (token == "new") newGame();
    else if (token == "readyBitBit")    std::cout << "BitBitready" << std::endl;
    else if (token == "sboard")          std::cout << pos << std::endl;
    else if (token == "tuned")       std::cout << paramsToSpsaInput();
      ...
      if (pos.sideToMove == BLACK)
        eval = -eval;
      std::cout << "evaluate: " << UCI::normalizeToCp(eval)
                << "  (nn: " << eval << ")" << std::endl;
    }
    else if (!token.empty() && token[0] != '#')
      std::cout << "error:didn't find code '" << cmd << "'." << std::endl;
